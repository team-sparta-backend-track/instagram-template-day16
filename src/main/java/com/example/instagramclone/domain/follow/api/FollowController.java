...
public class FollowController {
    ...
    public ResponseEntity<ApiResponse<SliceResponse<FollowMemberResponse>>> getFollowers(
            ...
            @LoginUser LoginUserInfoDto loginUser) {
        Pageable pageable = PageableUtil.createSafePageableDesc(page, size, "createdAt");
        SliceResponse<FollowMemberResponse> response = followService.getFollowers(loginUser.id(), memberId, pageable);
        return ResponseEntity.ok(ApiResponse.success(response));
    }
    ...
    public ResponseEntity<ApiResponse<SliceResponse<FollowMemberResponse>>> getFollowings(
            ...
            @LoginUser LoginUserInfoDto loginUser) {
        Pageable pageable = PageableUtil.createSafePageableDesc(page, size, "createdAt");
        SliceResponse<FollowMemberResponse> response = followService.getFollowings(loginUser.id(), memberId, pageable);
        return ResponseEntity.ok(ApiResponse.success(response));
    }
}
